...
public class UnfoldingBCSTerzo {
	...
	private void visitQueue() throws InterruptedException {
		while (!queue.isEmpty()) {
			...
			LocalConfiguration localConfigPetri = new LocalConfiguration();// petrinet
			for (Transition tlcm : localConfig.get()) {
				localConfigPetri.addAll((Transition) unf2PetriMap.get(tlcm));
			}
			System.out.println("LocalConfigurationPetri " + localConfigPetri);
			ArrayList<PetrinetNode> markingCmenoT = Utility.getMarking(petrinet, localConfigPetri);
			ArrayList<PetrinetNode> markingUnfolding = Utility.getMarking(unfolding, localConfig);
				...
				if (!presetAbilitato.isEmpty() && !markingUnfolding.isEmpty()) {
					...
					if (petri2UnfMap.containsKey(t1)) {
						...
						for (int j = 0; j < t1Unf.size() && !stessaHistory; j++) {
							...
							if (!Utility.isEnabledFromMarking(markingUnfolding, nodoStessaHistory, unfolding).isEmpty()) {
					...
					//	localConfigPetri.addAll((Transition) t1);

						// boolean isCutoff = isCutoff(localConfigPetri,
						// (Transition) t1, t2);
						if (t1.equals(reset)) {
							...
							if (isCutoff == false) {
								...
								LocalConfiguration localConfigNew = new LocalConfiguration();
								localConfigNew.set(unfolding, t2);
						...
						for (Transition tra : localConfig.get()) {
							ll.addAll(tra);
						}
